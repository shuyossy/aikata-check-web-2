...
本アプリの特徴は以下の通り
- プロジェクト単位でレビュー内容を共有
  - プロジェクトには複数のレビュースペースを作成することができる
- チェックリスト登録
  - レビュースペースごとに専用のチェックリストを登録することができる
  - 手動、AIチェックリスト生成によるチェックリストの登録が可能
- レビュー実行
  - レビュースペースに登録されたチェックリストに対してレビュー対象のレビューが可能
...
- プロンプトは英語で記載すること
  ...
  - 一般的で自然な英語表現にすること
- コードのコメントは日本語で記載すること
- ユーザメッセージは全て`messages/ja/template.ts`に集約し、日本語で記載すること
  - サーバサイドのエラーについては`next-server/lib/server/error.ts`に定義してある`AppError`をthrowすると最終的に`next-server/lib/server/baseAction.ts`でエラーを正規化してユーザにエラーメッセージを通知できる
  - クライアントサイドについては、直接`getMessage`を呼び出して、ユーザメッセージを組み立てること
...
- DBのマイグレーションコードは必ず`drizzle-kit`で作成すること
