...
export interface ApiReviewProgress {
  ...
  /** 現在のステータス */
  status: "idle" | "starting" | "processing" | "completing" | "completed" | "error";
  /** エラーメッセージ */
  errorMessage?: string;
}
...
export function useApiReview() {
  ...
  // サーバーアクション
  const {
    executeAsync: startReview,
    isPending: isStarting,
  } = useAction(startApiReviewAction);

  const {
    executeAsync: saveResults,
    isPending: isSaving,
  } = useAction(saveApiReviewResultsAction);

  const {
    executeAsync: completeReview,
    isPending: isCompleting,
  } = useAction(completeApiReviewAction);
  ...
  const callExternalApi = useCallback(async (
    ...
    const request = buildExternalReviewRequest(documents, checkListItems, reviewSettings);
    ...
    if (!response.ok) {
      ...
      throw new Error(`External API call failed: ${response.status} ${response.statusText} - ${errorText}`);
    }
   ...
   * 外部APIレビューを実行
   */
  const execute = useCallback(async (input: ApiReviewInput): Promise<{
    ...
    try {
      ...
      const { reviewTargetId, checkListItems, concurrentReviewItems } = startResult.data;

      // 2. チェックリストを分割
      const chunks = splitChecklistIntoChunks(checkListItems, concurrentReviewItems);
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        try {
          // チェック項目を外部API用の形式に変換
          const externalCheckListItems: ExternalReviewCheckListItem[] = chunk.items.map((item) => ({
            id: item.id,
          ...
          // 成功/エラー結果をカウント
          const successCount = apiResponse.results.filter((r) => !r.error).length;
          const errorCount = apiResponse.results.filter((r) => r.error).length;
          ...
          const saveResult = await saveResults({
            ...
            results: apiResponse.results.map((result) => {
              // チェック項目IDからコンテンツを取得
              const checkItem = chunk.items.find((item) => item.id === result.checkListItemId);
              return {
  ...
  return {
    ...
    progress,
    /** 実行中かどうか */
    isExecuting: isStarting || isSaving || isCompleting || progress.status === "processing",
    /** 進捗をリセット */
    reset,
  };
}
