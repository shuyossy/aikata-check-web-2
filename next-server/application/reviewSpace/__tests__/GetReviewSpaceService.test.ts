...
import { GetReviewSpaceService } from "../GetReviewSpaceService";
...
describe("GetReviewSpaceService", () => {
  let mockReviewSpaceRepository: IReviewSpaceRepository;
  let mockProjectRepository: IProjectRepository;
  let service: GetReviewSpaceService;
  ...
  beforeEach(() => {
    ...
    service = new GetReviewSpaceService(
      mockReviewSpaceRepository,
      mockProjectRepository,
    );
  });
  ...
  describe("正常系", () => {
    it("レビュースペースを取得できる", async () => {
      const result = await service.execute({
        reviewSpaceId: validReviewSpaceId,
        userId: validUserId,
      });

      expect(result.id).toBe(validReviewSpaceId);
