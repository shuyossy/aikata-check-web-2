...
export type LargeDocumentReviewInput = z.infer<typeof largeDocumentReviewInputSchema>;
...
export type LargeDocumentReviewOutput = z.infer<typeof largeDocumentReviewOutputSchema>;
...
type IndividualDocumentReviewRetryInput = z.infer<typeof individualDocumentReviewRetryInputSchema>;

/**
 * 個別ドキュメントレビューワークフローの出力スキーマ
 */
const individualDocumentReviewWorkflowOutputSchema = baseStepOutputSchema.extend({
  /** 元のファイル名 */
...
type IndividualDocumentReviewWorkflowOutput = z.infer<typeof individualDocumentReviewWorkflowOutputSchema>;
  ...
  .dountil(
    ...
    createWorkflow({
      ...
      .map(async ({ inputData, getInitData }) => {
        const initData = (await getInitData()) as IndividualDocumentReviewRetryInput;
        ...
        if (inputData.every((item) => item.status === "success")) {
          return {
            ...
            reviewResults: inputData.flatMap((item) => item.reviewResults || []),
          } as IndividualDocumentReviewRetryInput;
        }
        ...
        if (!hasContentLengthError) {
          ...
          return {
            ...
            finishReason: "error" as const,
            status: "failed" as const,
            errorMessage: errorItem?.errorMessage || formatMessage("REVIEW_UNEXPECTED_ERROR"),
            reviewResults: inputData.flatMap((item) => item.reviewResults || []),
          } as IndividualDocumentReviewRetryInput;
        }
        ...
        } else if (originalFile.imageData && originalFile.imageData.length > 0) {
          // 画像ドキュメントの分割
...
export const largeDocumentReviewWorkflow = createWorkflow({
  ...
  .map(async ({ inputData, bail, getInitData, runtimeContext }) => {
    ...
    if (failedItem) {
      // チェック項目全てにエラー結果を作成
      const errorResults: SingleReviewResult[] = initData.checkListItems.map(item => ({
        checkListItemContent: item.content,
        evaluation: null,
        comment: null,
        errorMessage: failedItem.errorMessage || formatMessage("REVIEW_UNKNOWN_ERROR"),
      }));
      ...
      return bail({
        status: "failed" as const,
        errorMessage: failedItem.errorMessage || formatMessage("REVIEW_INDIVIDUAL_DOC_REVIEW_FAILED"),
      });
    }
    ...
    for (const doc of documentsWithReviewResults) {
      ...
      const matchingItem = inputData.find(
        (item) =>
