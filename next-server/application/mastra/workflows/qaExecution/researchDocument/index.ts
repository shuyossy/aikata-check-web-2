...
import { checklistResultWithIndividualSchema, QaExecutionWorkflowRuntimeContext } from '../types';
import {
  getTotalChunksStep,
  getTotalChunksStepInputSchema,
} from '../steps/getTotalChunksStep';
import { researchChunkStep, researchChunkStepInputSchema } from '../steps/researchChunkStep';
import { splitTextByCount, splitImagesByCount } from '@/application/mastra/lib/util';
...
export type ResearchDocumentWithRetryInput = z.infer<typeof researchDocumentWithRetryInputSchema>;

/**
 * リトライ付きドキュメント調査ワークフローの出力スキーマ
 */
export const researchDocumentWithRetryOutputSchema = baseStepOutputSchema.extend({
  /** ドキュメントキャッシュID */
...
export type ResearchDocumentWithRetryOutput = z.infer<typeof researchDocumentWithRetryOutputSchema>;
  ...
  .dountil(
    createWorkflow({
      ...
      .map(async ({ inputData }) => {
        ...
        if (!documentCache) {
          throw new Error(`ドキュメントキャッシュが見つかりませんでした: ${documentCacheId}`);
        }
        ...
        if (documentCache.cachePath) {
          if (documentCache.isTextMode()) {
            textContent = await ReviewCacheHelper.loadTextCache(documentCache.cachePath);
          } else if (documentCache.isImageMode()) {
            imageData = await ReviewCacheHelper.loadImageCache(documentCache.cachePath);
          }
        }
      ...
      .map(async ({ inputData, bail, getInitData }) => {
        const initData = (await getInitData()) as z.infer<typeof chunkResearchInnerWorkflowInputSchema>;
        ...
        if (!hasContentLengthError && results.some((result) => result.status === 'failed')) {
        ...
        if (hasContentLengthError) {
          // チャンク数を増やして再試行
          logger.info(
            { documentCacheId: initData.documentCacheId, newTotalChunks: initData.totalChunks + 1 },
