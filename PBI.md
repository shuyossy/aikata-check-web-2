...
- 指摘事項
  - 本システムの特性上、レビュー対象のレビュー実行後にチェックリストが変更されることがある。レビュー結果にチェックリストIDを紐づけると、該当チェックリストが削除された際に外部キーエラーになってしまうのではないか
    - チェックリスト項目のテキスト内容をそのまま保存するか、チェックリストは論理削除にした方が良いのではないか
    - 今後レビュー対象毎のリトライ機能も実装するので、それも踏まえて最適な設計にしてください
    - workflowについてもそれに合わせて修正が必要であれば実施してください
...
- 注意事項
  - electron版にも同様の仕組みがあるので必要あれば参考にすること
...
- 受け入れ基準
  - レビューリトライ画面が設定されている（`12b-review-retry`参考）
    - リトライ範囲とレビュー設定を指定できる
      - レビュー設定は前回のレビュー設定がデフォルト値として表示されており、編集可能
  ...
  - レビュー実行毎に、リトライに必要な情報（チェックリスト項目、レビュー設定、初回レビュー時にアップロードされたドキュメント前処理内容（テキスト抽出・画像変換）のキャッシュ）が保存されていること
...
- 受け入れ基準
  - 並列実行数を環境変数で制御できる(デフォルトは1)
  - レビュー対象のレビューステータスに`待機中`が追加され、レビュー対象一覧セクションのステータスで表示される
- 注意事項